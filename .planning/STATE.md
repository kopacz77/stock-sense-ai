...
| 2026-08-30 | D-25 + LLM guard | CATALYST_ANCHORED target widened 2×→3×ATR_5 so gross R:R (2.0) clears the 1.5 net cost hurdle (operator choice over lowering the bar). LLM guard shipped 08-28 (`5d49902`): unload after each LLM cycle + quiet hours 22:00–07:00 ET, after the RDNA4 VRAM-eviction crash diagnosis; operator will move LM Studio to the CPU backend (DeepSeek deferred). Phase 11 incl. gap plan 11-09 complete; main pushed. |
