...
| Current Phase | M2-05 AI-Augmented Strategy Engine — **planning** (deferral window over; 11-RESEARCH.md committed 2026-08-27, PLAN pending) |
| Status | **Scorer outage found and fixed 2026-08-27.** LM Studio was off from ~2026-07-26 → 08-27; 15,343 articles queued in `score-backlog.jsonl`, every `ticker-day-summary` row in that window has `articleCount: 0` (PM-only). Scheduler + digests ran the whole time, so nothing looked wrong from Telegram. Shipped on `feat/scorer-health-backlog-drain`: `intel backlog-drain [--max N] [--batch N] [--manage-server]` (files back-scored records into their publish day + rebuilds those days' rollups; lock so it coexists with the scheduler; Ctrl-C safe; `--manage-server` starts/stops LM Studio via `lms.exe` interop for "as needed" RAM use), scorer-health line in every digest (⚠️ when backlog non-empty and nothing scored in 24h), `scored=/backlogged=/backlog=` on every cycle log line + `SCORER DOWN` warning, in-cycle drain now also buckets by publish day. Smoke: 20 articles drained in 0.8 min, 07-24 rollup rebuilt. 380/380 tests, `tsc` clean. Branch `fix/llm-correlator-retry-provenance` merged (ff) into main. See `docs/LM_STUDIO_OPS.md`. |
...
2. `/gsd-plan-phase 11` — plan M2-05 against the data reality above (RESEARCH.md §backtest options: Option B+C hybrid still stands; the corpus is now 36 scored days + PM continuous, not 30 clean days).
