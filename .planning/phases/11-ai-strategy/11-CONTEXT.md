...
### After-tax, after-fees hurdle on targets (operator request, 2026-08-28 — captured mid-execution, to be planned as a gap plan after phase verification)
- **Operator's words:** "we need to factor in how much it would have to grow to be a net gain when factoring in tax in Ontario Canada, or in California USA, as well as average fees when doing buy and sell interactions. so the sell point threshold has to be high enough to be able to handle that."
- **Decision:** every ranked candidate's target must clear a **net hurdle**, computed from a new `config/strategy-config.json` block: `costs.jurisdiction` (`"ON-CA"` | `"CA-US"`), `costs.marginalRatePct` (operator-set; the engine never guesses a tax bracket), `costs.capitalGainsInclusionPct` (Ontario: 50% inclusion of the gain at the marginal rate; California: short-term gains taxed as ordinary income — federal + state — so inclusion 100%), `costs.perTradeFeeUsd` (round-trip = 2×), `costs.spreadSlippageBps` (default 5 bps each side, matching M2-01's FixedBPSSlippageModel). 
- **Math to encode (not a guess — this is how the numbers work):** fees and slippage move the *break-even*: `minGrossMovePct = (2 × perTradeFeeUsd / sizeUsd) + 2 × spreadSlippageBps/10000`. Tax does NOT move break-even (a taxed gain is still a gain) but it scales the *reward*: `afterTaxReward = grossReward × (1 − marginalRate × inclusion)` while the risk (stop distance) stays pre-tax (losses offset gains only against other gains). So the target must satisfy BOTH: (a) `target − entry ≥ entry × minGrossMovePct` and (b) `afterTaxReward / risk ≥ config.minRewardRisk` (new key, default 1.5). Candidates that fail either are demoted below the 0.4 floor with an explicit `costs` reason in the rationale — never silently re-targeted upward, because a wider target is a different trade.
- **Surface:** `strategy run` prints per-candidate `net R:R` and the hurdle; `strategy backtest` reports gross AND after-cost Sharpe/MaxDD; decision log records `afterTaxRewardUsd` on accept.
- **Boundary:** this is a cost model for *ranking/level* decisions, not tax accounting — tax-lot tracking stays in M2-07.
- **Refinement (operator, 2026-08-28):** the operator will move from Ontario to California "in due time", so `costs.jurisdiction` is a genuine runtime selection and BOTH rule sets must be identified completely in the plan — not one rate knob with a label. Each jurisdiction profile (`config/tax-profiles.json`, one object per jurisdiction, operator-editable, every number annotated with its source and a "confirm with your accountant" flag) must cover at least:
  - **Gain characterisation for swing trades:** ON-CA — capital gain, inclusion rate (currently 50% for most individuals; the plan must record the rate as a dated config value, not hard-code it, given the 2024–25 inclusion-rate changes), taxed at the combined federal+Ontario marginal rate; CRA may treat frequent trading as *business income* (100% inclusion) — surface as a documented risk flag, not a computed path. CA-US — short-term (≤ 1 year) gains taxed as ordinary income at federal marginal rate + California marginal rate (California gives no preferential capital-gains rate); long-term (> 1 year) federal preferential rates are irrelevant to swing horizons but must be selectable by holding period; Net Investment Income Tax 3.8% above the federal threshold as a toggle.
  - **Loss rules that change net expectancy:** US **wash-sale** (30-day) disallowance and the Canadian **superficial-loss** (30-day) rule — both matter for a strategy that re-enters the same ticker after a stop-out; the engine should at minimum *flag* a candidate whose ticker was closed at a loss within the trailing 30 days, and the backtest's after-cost path should model the disallowed loss.
