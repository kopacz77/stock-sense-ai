...
## From 11-07 (minimal /strategy web route)

**3. `GET /api/strategy/candidates`'s `skippedTypes` field is always `[]`.**
- **Found during:** Task 1 implementation (`src/web/server.ts`), before the checkpoint.
- **File:** `src/web/server.ts` (this plan's own new endpoint) / `src/strategy/strategy-engine.ts` (pre-existing, untouched by this plan).
- **Issue:** `StrategyRunResult.skippedTypes` (signal type + gate-reason pairs, e.g. SENTIMENT_VELOCITY's coverage-gap message) is a run-time-only field the CLI prints from its own in-process `generateCandidates()` call. `StrategyEngine` never persists it to `candidates-YYYY-MM-DD.jsonl`, so the web endpoint — which deliberately reads the persisted file rather than re-running the engine (T-11-07-05, DoS mitigation) — has no source to read `skippedTypes` from and always returns `[]`. Cosmetic only: the web page's skipped-types section always renders empty even on a day where, e.g., SENTIMENT_VELOCITY was gated off.
- **Why deferred:** Fixing it means `StrategyEngine.generateCandidates` (or its CLI caller) additionally persisting `skippedTypes` to disk per run — a change to 11-05's engine/CLI, out of this plan's `files_modified` (`src/web/server.ts` and the frontend files only).
- **Suggested owner:** A small follow-up to `strategy-engine.ts` (or a sibling `skipped-types-YYYY-MM-DD.jsonl` write next to `candidates-*.jsonl`) the next time the engine/CLI files are touched.
