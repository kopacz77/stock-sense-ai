...
| 11-09-02 | 11-09 | 6 | INCOME-01 (ext) — wash-sale / superficial-loss flag from the decision log (trailing 30 days, same ticker, closed at a loss) + `afterTaxRewardUsd` on accept | T-11-09-05, T-11-09-06 | decision log read once per run, not per candidate; accept never lost to a cost-config failure | unit | `pnpm vitest run src/strategy/__tests__/costs.test.ts src/strategy/__tests__/decision-log.test.ts` | ⬜ | ⬜ pending |
| 11-09-03 | 11-09 | 6 | INCOME-01 (ext) — backtest reports gross AND after-cost (incl. tax + disallowed loss); net R:R on the API and the web card | T-11-09-01, T-11-09-07, T-11-09-SC | `redactCostEvaluation` allowlist keeps the operator's tax rate out of the HTTP response; zero new packages | integration | `pnpm vitest run src/strategy/backtest/__tests__/live-window-runner.test.ts src/web/__tests__/server-strategy-decision.test.ts` | ⬜ | ⬜ pending |
